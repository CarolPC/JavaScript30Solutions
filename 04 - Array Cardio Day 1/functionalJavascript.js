...
const inventorsBefore1500 = inventors.reduce((aggr, inventor) => {
    if (inventor.year >= 1500 && inventor.year < 1600) {
        return aggr.concat(inventor);
    }
    return aggr;
}, []);
...
const inventorsYearsLived = inventors.reduce((sum, inventor) => {
...
const timesEachTransportation = data.reduce((aggr, transportation) => {
     if (Object.keys(aggr).includes(transportation)){
         aggr[transportation] += 1;
     }
     else {
        aggr[transportation] = 1;
     }
...
console.log(timesEachTransportation);
